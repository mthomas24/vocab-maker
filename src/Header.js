import { Modal, newTerm, TextInput, Toggle } from "./Components";
import { useState } from "react";
import { MdAdd, MdClearAll, MdDownload, MdFilterList, MdUpload } from "react-icons/md";
...
export default function Header({ setTerms, setBulkOpen, setIsExporting }) {
  ...
  return <header className="flex justify-end items-stretch gap-3 p-3 bg-emerald-500 text-white">
    ...
    <Modal open={clearOpen} onClose={() => setClearOpen(false)} classNames="m-4">
      <div className="text-neutral-800">
        <p className="mb-6">Are you sure you want to delete all the terms? This action cannot be undone.</p>
        <div className="flex justify-end items-stretch gap-3">
          <button className="font-semibold hover:underline" onClick={() => setClearOpen(false)}>Cancel</button>
          <button
    ...
    <HeaderButton
      ...
      Icn={MdUpload}
    />
    <HeaderButton onClick={() => setIsExporting(true)}
      txt="Export"
      Icn={MdDownload}
    />
  </header>
